...
def is_docker_image_available():
    ...
    stdout, stderr = subprocess.Popen(
        ["docker", "images", "-q", "%s:%s" % (DOCKER_IMAGE, DOCKER_TAG)],
...
def run_in_docker(file, paths, ports, args):
    ...
    arg_list += ["sh", "-c", "export PYTHONPATH=$PYTHONPATH:/synbols_include;" + f"python '{file}' {args}"]
...
def main():
    ...
    parser.add_argument("--docker-port", type=int, nargs="+", help="Port to be opened at run time")
    ...
    if not is_docker_installed():
        print(
            "Error: Docker installation not found. "
    ...
    if not is_docker_image_available():
        print(
            "The Synbols Docker image for package version" + f"{SYNBOLS_VERSION} is not available on this system. It",
    ...
    run_in_docker(args.file, paths=args.mount_path, ports=args.docker_port, args=script_args)
