...
if __name__ == '__main__':
    ...
    font_matrix = csr_matrix(
        (max(b['stop'] for b in unicode_blocks.values()), len(fonts)), dtype=np.uint8)
    ...
    save_npz('font_glyph_availability.npz', font_matrix)

    print(np.argsort(font_matrix.sum(axis=1)).reshape(-1,))
